...
prior to `# sensu-tor-check` from the generated README file, and add additional context about the
...
The sensu-tor-check is a [Sensu Check][6] that ...
...
If you're using an earlier version of sensuctl, you can find the asset on the [Bonsai Asset Index][https://bonsai.sensu.io/assets/elfranne/sensu-tor-check].
...
metadata:
  ...
  namespace: default
spec:
  command: sensu-tor-check --example example_arg
  subscriptions:
  - system
  runtime_assets:
...
[4]: https://github.com/elfranne/sensu-tor-check/blob/master/.github/workflows/release.yml
[5]: https://github.com/elfranne/sensu-tor-check/actions
